  ...
  return (
    <div className="w-full bg-[#2563eb] text-white p-6 rounded mb-6 mt-14">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <div>
          <div className="text-xl font-semibold font-heading">Good afternoon, {firstName}!</div>
        ...
        <div className="flex gap-3">
            <button onClick={onSearch} className="bg-white/10 hover:bg-white/20 px-3 py-2 rounded flex items-center gap-2"><Search size={16} /> Find Vendors</button>
            <button onClick={onOpenFavorites} className="bg-white/10 hover:bg-white/20 px-3 py-2 rounded flex items-center gap-2"><Heart size={16} /> My Favorites</button>
        </div>
      </div>
    </div>
  )
